...
- `visual-lox-editor` builds the `vlox-editor` executable, containing the ImGui editor and file dialogs.
- `visual-lox-cli` builds the `vlox` executable, a console runner for `.vlox` files that does not start the editor.
- `visual-lox-application` builds the `vlox-app` executable, an ImGui host for interactive `.vlox` applications.
...
cmake --build build --config Debug --target visual-lox-editor visual-lox-cli visual-lox-application visual-lox-tests
```
...
```sh
build/bin/vlox_d.exe path/to/script.vlox
```
...
The `UI` node category provides text, buttons, number inputs and sliders, layout helpers, and an image widget. `UI::Image`, which uses a list of row-major scalar pixels as an input.

Use `--disassemble` to print the generated bytecode before executing it:

```sh
build/bin/vlox_d.exe --disassemble path/to/script.vlox
```
