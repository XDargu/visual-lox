...
ComponentLayout LayoutComponent(const std::vector<size_t>& componentNodes, const std::vector<GraphLayout::Node>& nodes, const std::vector<EdgeRecord>& edges, const GraphLayout::Options& options)
{
...
namespace
{
std::vector<GraphLayout::Position> CalculateLayered(const std::vector<GraphLayout::Node>& inputNodes, const std::vector<GraphLayout::Edge>& inputEdges, const GraphLayout::Options& options)
{
                               ...
                               const std::vector<GraphLayout::Edge>& flowEdges, const std::map<int, DataCluster>& clusters)
{
    ...
    for (const auto& [nodeId, neighbors] : undirected)
    {
        ...
        while (outgoing[current].size() == 1)
        {
            ...
            const float desiredY = positions[positionById.at(current)].value.y + (edge.hasPinOffsets ? edge.sourceOffsetY - edge.targetOffsetY : 0.0f);
